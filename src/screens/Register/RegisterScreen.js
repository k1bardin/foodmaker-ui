...
const { navigation, route } = props;
 ...
 try {
 // Валидация полей
 if (!formData.firstName || !formData.lastName || !formData.email || !formData.password || !formData.typeId) {
 alert('Пожалуйста, заполните все обязательные поля');
 return;
 }

 // Регистрация
 const registerResponse = await fetch('http://192.168.88.249:8082/register', {
 method: 'POST',
 headers: {
 ...
 // Переход на следующий экран или сохранение токена
 console.log('Регистрация успешна:', loginResult);
 } catch (error) {
 ...
 <TouchableOpacity
 key={type.typeId}
 style={[styles.chip, selectedTypeId === type.typeId ? styles.chipSelected : null]}
